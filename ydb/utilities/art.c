// Source: https://github.com/armon/libart
...
static void destroy_node(art_node *n, void (*data_free)(void *)) {
    // Break if null
    if (!n) return;
    ...
    switch (n->type) {
        case NODE4:
            ...
            for (i=0;i<n->num_children;i++) {
                destroy_node(p.p1->children[i], data_free);
            }
            break;
        ...
        case NODE16:
            p.p2 = (art_node16*)n;
            for (i=0;i<n->num_children;i++) {
                destroy_node(p.p2->children[i], data_free);
            }
            break;
        ...
        case NODE48:
            ...
            for (i=0;i<256;i++) {
                ...
                if (!idx) continue; 
                destroy_node(p.p3->children[idx-1], data_free);
            }
            break;
        ...
        case NODE256:
            ...
            for (i=0;i<256;i++) {
                if (p.p4->children[i])
                    destroy_node(p.p4->children[i], data_free);
            }
            break;
...
int art_tree_destroy(art_tree *t) {
    destroy_node(t->root, NULL);
    return 0;
}

int art_tree_destroy_custom(art_tree *t, void (*data_free)(void *)) {
    destroy_node(t->root, data_free);
    return 0;
}
...
static int recursive_iter_leaf(art_node *n, art_callback_leaf cb, void *data) {
    ...
    switch (n->type) {
        case NODE4:
            for (int i=0; i < n->num_children; i++) {
                res = recursive_iter_leaf(((art_node4*)n)->children[i], cb, data);
                if (res) return res;
            }
            break;
        ...
        case NODE16:
            for (int i=0; i < n->num_children; i++) {
                res = recursive_iter_leaf(((art_node16*)n)->children[i], cb, data);
                if (res) return res;
            }
            break;
        ...
        case NODE48:
            for (int i=0; i < 256; i++) {
                ...
                res = recursive_iter_leaf(((art_node48*)n)->children[idx-1], cb, data);
                if (res) return res;
            }
            break;
        ...
        case NODE256:
            for (int i=0; i < 256; i++) {
                if (!((art_node256*)n)->children[i]) continue;
                res = recursive_iter_leaf(((art_node256*)n)->children[i], cb, data);
                if (res) return res;
            }
            break;
...
int art_iter_prefix_leaf(art_tree *t, const unsigned char *key, int key_len, art_callback_leaf cb, void *data) {
    art_node **child;
    art_node *n = t->root;
    int prefix_len, depth = 0;
    while (n) {
        ...
        if (depth == key_len) {
            art_leaf *l = minimum(n);
            if (!leaf_prefix_matches(l, key, key_len))
               return recursive_iter_leaf(n, cb, data);
            return 0;
        }
