...
static char *_ymldb_opcode_str(int opcode)
{
    ...
    if (opcode & YMLDB_OP_LOCAL)
        strcat(opstr, "local|");
    if (opcode & YMLDB_OP_REMOTE)
        strcat(opstr, "remote|");
    return opstr;
}
...
void ymldb_dump_start(FILE *stream, int opcode, int sequence)
{
    ...
    if (opcode & YMLDB_OP_LOCAL)
    {
        _out(stream, "%s %s %s\n", "%TAG", YMLDB_TAG_OP_LOCAL, YMLDB_TAG_LOCAL);
    }
    if (opcode & YMLDB_OP_PUBLISHER)
    {
...
int _ymldb_merge(struct ymldb_cb *cb, struct ymldb *p_ydb, int index, int p_index)
{
    ...
    if (!p_ydb) {
        _log_error("merge failed - unknown p_ydb\n");
        cb->res = -1;
        return -1;
    }
...
int _ymldb_op_extract(struct ymldb_cb *cb)
{
    ...
    if (cb->document->tag_directives.start != cb->document->tag_directives.end)
    {
             ...
             tag != cb->document->tag_directives.end; tag++)
        {
            ...
            else if (strcmp(op, YMLDB_TAG_OP_LOCAL) == 0)
            {
                opcode = opcode | YMLDB_OP_LOCAL;
            }
...
int _ymldb_run(struct ymldb_cb *cb, FILE *instream)
{
    ...
    while (!done)
    {
        ...
        {
            cb->sequence++;
            _log_debug("msg no: %dth\n", cb->sequence);
            _log_debug("opcode: %s\n", _ymldb_opcode_str(in_opcode));
           
            if(_ymldb_op_ignored(cb, in_opcode)) {
                _log_debug("document igonred!\n");
                goto skip_document;
            }
            
            cb->opcode =
                (cb->flags & YMLDB_FLAG_PUBLISHER) ? YMLDB_OP_PUBLISHER : 
                (cb->flags & YMLDB_FLAG_SUBSCRIBER) ? YMLDB_OP_SUBSCRIBER : 0;
            cb->opcode = cb->opcode 
                | (in_opcode & (YMLDB_OP_MERGE | YMLDB_OP_DELETE | YMLDB_OP_GET));
...
struct ymldb_cb *ymldb_create(char *key, unsigned int flags)
{
    ...
    cb->document = NULL;
    cb->flags = flags & (YMLDB_FLAG_PUBLISHER | YMLDB_FLAG_SUBSCRIBER);
    cb->outstream = NULL;
    cb->last_notify = NULL;
    cb->no_reply = 0;
    // cb->fd_local = 0;
    ...
    if (cb->fd_publisher)
    {
        if (FD_ISSET(cb->fd_publisher, set))
        {
            ...
            { // SUBSCRIBER
                ...
                FD_CLR(cb->fd_publisher, set);
                ymldb_run_with_string(cb, inbuf, inlen);
                free(inbuf);
            }
        }
    }
...
static int _ymldb_conn_send(struct ymldb_cb *cb)
{
    ...
    {
    subscriber_rewrite:
        res = write(cb->fd_publisher, cb->outbuf + sent, ftell(cb->outstream) - sent);
        if (res < 0)
        {
            cb->flags |= YMLDB_FLAG_RECONNECT;
    ...
    else if (cb->flags & YMLDB_FLAG_PUBLISHER)
    {
        for (int i = 0; i < YMLDB_SUBSCRIBER_MAX; i++)
        {
            if (cb->fd_subscriber[i])
            {
                ...
                retry = 0;
            publisher_rewrite:
                res = write(cb->fd_subscriber[i], cb->outbuf + sent, ftell(cb->outstream) - sent);
                if (res < 0)
                {
...
//         return -1;
// rewrite:
//     res = write(cb->fd_local, cb->outbuf + sent, ftell(cb->outstream) - sent);
//     if (res < 0)
//     {
...
int ymldb_run_with_string(struct ymldb_cb *cb, char *ymldata, size_t ymldata_len)
{
    ...
    if (instream)
    {
        int res = _ymldb_run(cb, instream);
        fclose(instream);
        return res;
    }
    else {
...
int ymldb_push(struct ymldb_cb *cb, int opcode, char *format, ...)
{
    ...
    return ymldb_run_with_string(cb, streambuf, strlen(streambuf));
}
...
int ymldb_pull(struct ymldb_cb *cb, char *format, ...)
{
    ...
    fclose(stream);
    // read ymldb data from the streambuf
    int res = ymldb_run_with_string(cb, streambuf, strlen(streambuf));
    if(res < 0) {
        return -1;
    }
    return 0;
...
int _ymldb_write(struct ymldb_cb *cb, int opcode, int num, ...)
{
    ...
    ymldb_dump_end(stream);
    fclose(stream);
    return ymldb_run_with_string(cb, ymldata, strlen(ymldata));
}
